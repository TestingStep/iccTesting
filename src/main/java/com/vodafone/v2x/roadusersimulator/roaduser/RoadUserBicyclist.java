...
public class RoadUserBicyclist extends RoadUser {
    private static Logger logger = LogManager.getLogger(RoadUserBicyclist.class);
    ...
    private VAMProfile vamProfile = VAMProfile.BICYCLIST;
    private boolean isDynamic;


    public RoadUserBicyclist(Gateway gateway, String geohashString, boolean isDynamic) {
        super(StationType.CYCLIST);
    ...
    private void initializeLocation() {
        ...
        if (isDynamic) {
            speedInKmH = (float) (Math.random() * 30d) + 1f;
        }else {
            speedInKmH=0f;
        }
    }
